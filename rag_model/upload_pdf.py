...
def upload_to_text_namespace(pdf_path: str, namespace: str):
    ...
    result = doc_service.upload_pdf(pdf_path=pdf_path, namespace=namespace)
...
def upload_to_vector_namespace(pdf_path: str, namespace: str):
    ...
    if not VECTOR_AVAILABLE:
        raise ValueError(
            "Vector namespace support requires google-generativeai package.\n"
        ...
        if embedding:
            vectors.append({
                ...
                "metadata": {
                    ...
                    "chunk_index": i,
                    "text": chunk[:200]  # Store first 200 chars as preview
                }
            })
        else:
            failed_count += 1
    ...
    if namespace_type == "vector":
        ...
        with MoorchehSDK(api_key=api_key) as client:
            try:
                ...
                existing = [ns for ns in ns_list.get('namespaces', []) 
...
def main():
    ...
    parser.add_argument("--type", choices=["text", "vector"], default="text",
                       help="Namespace type (default: text)")
